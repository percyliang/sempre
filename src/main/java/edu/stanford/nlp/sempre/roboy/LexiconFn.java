...
public class LexiconFn extends SemanticFn {
  ...
  private DbFormulasInfo dbFormulaInfo;
  ...
  public LexiconFn() throws IOException {
    ...
    dbFormulaInfo = DbFormulasInfo.getSingleton();
  }
      ...
      switch (mode) {
        ...
        case "binary": {
          ...
          for (LexicalEntry entry : entries) {
            if (!dbFormulaInfo.isCvt(((BinaryLexicalEntry) entry).expectedType1)
                    && !dbFormulaInfo.isCvt(((BinaryLexicalEntry) entry).expectedType2))
  ...
  private static Set<Pair<String, Formula>> collectLexemeFormulaPairs(Example ex) {
    ...
    for (Pair<String, Formula> pair: temp) {
      if (!BinaryLexicon.getInstance().validBinaryFormula(pair.getSecond())) {
        pair.setSecond(DbFormulasInfo.getSingleton().equivalentFormula(pair.getSecond()));
      }
      res.add(pair);
    }
    return res;
