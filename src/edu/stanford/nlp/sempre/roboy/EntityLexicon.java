...
import edu.stanford.nlp.sempre.roboy.index.DbEntitySearcher;
import edu.stanford.nlp.sempre.roboy.index.DbIndexField;
import edu.stanford.nlp.sempre.roboy.lexicons.EntrySource;
import edu.stanford.nlp.sempre.roboy.lexicons.LexicalEntry;
import edu.stanford.nlp.sempre.roboy.lexicons.LexicalEntry.EntityLexicalEntry;
...
public final class EntityLexicon {
  ...
  DbEntitySearcher exactSearcher;  // Lucene
  DbEntitySearcher inexactSearcher;  // Lucene

  DatabaseSearch freebaseSearch;  // Google's API
  ...
  public List<EntityLexicalEntry> lookupEntries(String query, SearchStrategy strategy) throws ParseException, IOException {
    ...
    switch (strategy) {
      case exact:
        if (exactSearcher == null) exactSearcher = new DbEntitySearcher(opts.exactMatchIndex, opts.numOfDocs, "exact");
        return lookupEntries(exactSearcher, query);
      case inexact:
        if (inexactSearcher == null) inexactSearcher = new DbEntitySearcher(opts.inexactMatchIndex, opts.numOfDocs, "inexact");
        return lookupEntries(inexactSearcher, query);
      case fbsearch:
        if (freebaseSearch == null) freebaseSearch = new DatabaseSearch();
        if (mid2idCache == null) mid2idCache = StringCacheUtils.create(opts.mid2idPath);
        return lookupDatabaseSearchEntities(query);
      default:
  ...
  public List<EntityLexicalEntry> lookupDatabaseSearchEntities(String query) {
    DatabaseSearch.ServerResponse response = freebaseSearch.lookup(query);
    ...
    for (DatabaseSearch.Entry e : response.entries) {
      ...
      entities.add(new EntityLexicalEntry(query, query, Collections.singleton(e.name),
              new ValueFormula<>(new NameValue(id, e.name)), EntrySource.DBSEARCH, e.score, distance,
              new FreebaseTypeLookup().getEntityTypes(id), entityFeatures));
    }
    return entities;
  }

  public List<EntityLexicalEntry> lookupEntries(DbEntitySearcher searcher, String textDesc) throws ParseException, IOException {
    ...
    for (Document doc : docs) {
      ...
      Formula formula = Formula.fromString(doc.get(DbIndexField.ID.fieldName()));
      String[] fbDescriptions = new String[]{doc.get(DbIndexField.TEXT.fieldName())};
      String typesDesc = doc.get(DbIndexField.TYPES.fieldName());
      ...
      double popularity = Double.parseDouble(doc.get(DbIndexField.POPULARITY.fieldName()));
