...
import edu.stanford.nlp.sempre.roboy.lexicons.normalizers.EntryNormalizer;
import edu.stanford.nlp.sempre.roboy.lexicons.normalizers.PrepDropNormalizer;
import fig.basic.*;
...
public final class BinaryLexicon {
  ...
  private DbFormulasInfo dbFormulasInfo;
  ...
  private BinaryLexicon() throws IOException {
    ...
    dbFormulasInfo = DbFormulasInfo.getSingleton();
  ...
  public List<BinaryLexicalEntry> buildEntry(LexiconValue lexValue, String lexemeKey) {
    ...
    BinaryFormulaInfo info = dbFormulasInfo.getBinaryInfo(lexValue.formula);
    ...
    if (dbFormulasInfo.isCvtFormula(info) && source == EntrySource.STRING_MATCH) {

      List<BinaryLexicalEntry> entries = new ArrayList<>();
      for (BinaryFormulaInfo cvtInfo : dbFormulasInfo.getCvtExpansions(info)) {
        entries.add(
  ...
  public void updateLexicon(Pair<String, Formula> lexemeFormulaPair, int support) {
    ...
    if (!exists) {
      BinaryFormulaInfo bInfo = dbFormulasInfo.getBinaryInfo(formula);
      if (bInfo == null) {
